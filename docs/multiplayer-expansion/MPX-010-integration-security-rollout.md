...
- 指标、告警、feature flag、灰度策略、应用回滚说明和 `docs/multiplayer.md`/README 发布状态更新；本地多人统计 v3→新 schema 的导入/聚合回归也纳入发布闸门。
- 准备并发布面向用户的更新公告：说明 N 人竞速、`playerLimit`、房间聊天/闭麦的使用方式，强调不必等到房间满员即可开局，并列出本轮仍不支持的组队、N 人接力、图片消息等范围。公告还需包含旧页面需要刷新、短期 v1 房间会排空/关闭、灰度期间功能可能分批开放等兼容提示。
- 公告在灰度前形成可评审草稿，在功能默认开放时通过项目既有用户公告渠道正式发布；若发布回滚或功能临时关闭，应同步更新公告状态，避免用户看到与线上能力不一致的说明。
...
- 用户公告已经评审并随默认开放同步发布，正文准确链接多人玩法说明，清楚区分“已经上线”“灰度中”和“暂不支持”的能力；公告链接和发布时间记录在发布清单中。发生回滚时，公告在同一发布流程中更新。
