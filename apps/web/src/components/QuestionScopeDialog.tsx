...
export function QuestionScopeDialog({
  ...
  const [characterFilterOpen, setCharacterFilterOpen] = useState(false);
  ...
  return (
    <div className="fixed inset-0 z-[2147483200] flex items-center justify-center bg-black/45 px-4 py-6">
      <ConfigProvider
        theme={{
          token: {
            borderRadius: 5,
      ...
      >
        <section
          className="question-scope-dialog flex max-h-[88vh] w-full flex-col overflow-hidden rounded-[8px] border border-line bg-paper shadow-xl"
          role="dialog"
          aria-modal="true"
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
            {loading ? (
              ...
              <div className="grid gap-4">
                <details className="question-scope-disclosure rounded-[6px] border border-line bg-paper-muted p-3">
                  <summary className="cursor-pointer text-sm font-black text-ink marker:text-vermilion">
                    预设难度选择
                  </summary>
                    ...
                    {QUESTION_DIFFICULTY_PRESETS.map((preset) => {
                      ...
                      return (
                        <button
                          ...
                          title={
                            available ? undefined : "当前题库暂无此难度角色"
                          }
                          className={`question-scope-preset min-h-[84px] rounded-[6px] border px-3 py-2.5 text-left transition ${
                            active
                  ...
                  <div className="mt-3 grid gap-2">
                    <div className="grid gap-2 md:grid-cols-3">
                      {(snapshot.fieldDefinitions ?? [])
                        ...
                        .map((definition) => {
                          const mode =
                            rules.fieldModes[definition.key] ??
                          ...
                          return (
                            <div
                              ...
                              className="question-scope-toggle-card flex min-h-[40px] items-center justify-between gap-2 rounded-[5px] border border-line bg-paper px-2.5 py-1.5"
                            >
                              <button
                                type="button"
                                disabled={readOnly}
                      ...
                      {snapshot.works.map((work) => {
                        ...
                        return (
                          <button
                            ...
                            type="button"
                            disabled={
                              readOnly || (state?.totalCount ?? 0) === 0
                            }
                            data-selected={checked === false ? "false" : "true"}
                            className="question-scope-filter-card flex min-h-[46px] items-center gap-2.5 rounded-[5px] border border-line bg-paper-muted px-2.5 py-1.5 text-left text-sm text-ink hover:bg-paper disabled:cursor-default disabled:opacity-60"
                            role="checkbox"
                            aria-checked={checked}
                ...
                <section className="question-scope-filter-separator">
                  <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                    <button
                      type="button"
                      className="flex flex-wrap items-center gap-2 text-left"
                      aria-expanded={characterFilterOpen}
                      onClick={() => setCharacterFilterOpen((value) => !value)}
                    >
                      {characterFilterOpen ? (
                        <ChevronDown size={16} aria-hidden="true" />
                      ) : (
                      ...
                      <span className="text-[0.76rem] font-bold text-ink-soft">
                        已选择 {draft.selectedCharacterIds.length}/
                        {answerableCount}
                      </span>
                    </button>
                    <div className="flex items-center gap-1.5">
                      ...
                      {sortedCharacters.map((character) => {
                        ...
                        return (
                          <button
                            ...
                            data-selected={checked ? "true" : "false"}
                            className="question-scope-filter-card grid min-h-[58px] grid-cols-[36px_minmax(0,1fr)_24px] items-center gap-2.5 rounded-[5px] border border-line bg-paper-muted px-2.5 py-1.5 text-left disabled:cursor-default disabled:opacity-60"
