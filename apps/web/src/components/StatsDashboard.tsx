...
export function StatsDashboard() {
  ...
  const incompleteDrafts = useLiveQuery(
    async () =>
      (await statsDb.drafts.where("kind").equals("multiplayer").toArray()).filter(
        (draft) => draft.kind === "multiplayer" && draft.incomplete,
      ).length,
    [revision],
    0,
  );

  useEffect(() => subscribeStatsChanges(() => setRevision((value) => value + 1)), []);

  const filtered = useMemo(() => filterStatsRecords(records, filters), [records, filters]);
  const metrics = useMemo(() => summarize(filtered), [filtered]);
  const works = useMemo(() => aggregateWorks(filtered), [filtered]);
  const guessDistribution = useMemo(() => winningGuessDistribution(filtered), [filtered]);
  const guessHistogram = useMemo(() => buildHistogram(guessDurations(filtered)), [filtered]);
  ...
  const workOption = useMemo<EChartsCoreOption>(() => ({
    ...
    grid: { left: 48, right: 48, top: 48, bottom: works.length > 10 ? 72 : 42 },
    ...
    xAxis: { type: "category", data: works.map((item) => item.code), axisLabel: { interval: 0 } },
    yAxis: [
    ...
    series: [
      ...
      { name: "胜率", type: "line", yAxisIndex: 1, data: works.map((item) => item.winRate), symbolSize: 7, smooth: 0.2 },
    ],
  }), [works]);
  ...
  const distributionOption = useMemo<EChartsCoreOption>(() => ({
    ...
    xAxis: { type: "category", name: "猜测次数", data: guessDistribution.map((item) => String(item.guesses)) },
    yAxis: { type: "value", minInterval: 1, name: "获胜题局" },
    series: [{ type: "bar", name: "获胜题局", data: guessDistribution.map((item) => item.count), barMaxWidth: 34 }],
  }), [guessDistribution]);

  const histogramOption = (bins: ReturnType<typeof buildHistogram>): EChartsCoreOption => ({
    grid: { left: 42, right: 16, top: 24, bottom: 48 },
    tooltip: { trigger: "axis" },
    xAxis: { type: "category", data: bins.map((bin) => bin.label), axisLabel: { rotate: bins.length > 6 ? 30 : 0 } },
    yAxis: { type: "value", minInterval: 1 },
    series: [{ type: "bar", data: bins.map((bin) => bin.count), barMaxWidth: 34 }],
  });

  const handleExport = async () => downloadStatsExport(await createStatsExport());
  ...
  const handleImportFile = async (file?: File) => {
    ...
    try {
      ...
      setImportPreview({ file: parsed, ...(await previewStatsImport(parsed.records)) });
    } catch (error) {
      setImportError(error instanceof Error ? `无法读取统计文件：${error.message}` : "无法读取统计文件。");
    } finally {
  ...
  const applyImport = async (mode: "merge" | "replace") => {
    ...
    if (
      mode === "replace" &&
      !window.confirm("覆盖导入会先清除现有统计记录，当前游戏进度不受影响。确定继续吗？")
    ) return;
  ...
  return (
    <main className="mx-auto w-full max-w-[1240px] px-[18px] pb-20 pt-8 max-[680px]:pb-24 max-[680px]:pt-5">
      <header className="flex flex-wrap items-end justify-between gap-5 border-b border-line pb-5">
        <div>
          <p className="mb-1 text-[0.72rem] font-bold uppercase text-vermilion">LOCAL STATS</p>
          <h1 className="font-brand text-[2.5rem] font-black text-ink max-[680px]:text-[2rem]">游玩统计</h1>
          <p className="mt-1 text-sm text-ink-soft">数据仅保存在此浏览器中。</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <input ref={fileInputRef} className="sr-only" type="file" accept="application/json,.json" onChange={(event) => void handleImportFile(event.target.files?.[0])} />
          <ActionButton icon={Upload} label="导出" onClick={() => void handleExport()} />
          <ActionButton icon={Download} label="导入" onClick={() => fileInputRef.current?.click()} />
          <ActionButton icon={Trash2} label="清除数据" danger onClick={() => setClearOpen(true)} />
        </div>
      </header>

      <section className="flex flex-wrap items-start gap-4 border-b border-line py-4" aria-label="统计筛选">
        <div className="grid gap-1.5">
          <span className="h-4 text-xs font-bold leading-4 text-ink-soft">模式</span>
          <div className="inline-flex h-10 rounded-[6px] border border-line bg-[var(--surface-soft)] p-1" role="group" aria-label="游戏模式">
        ...
        <DateRangeFilter
          from={filters.from ?? ""}
          to={filters.to ?? ""}
          onFromChange={(from) => setFilters((current) => ({ ...current, from: from || undefined }))}
          onToChange={(to) => setFilters((current) => ({ ...current, to: to || undefined }))}
          onClear={() => setFilters((current) => ({ ...current, from: undefined, to: undefined }))}
        />
        <label className="grid gap-1.5">
          <span className="h-4 text-xs font-bold leading-4 text-ink-soft">多人赛制</span>
      ...
      {importError ? <p className="mt-4 rounded-[5px] border border-[var(--error-border)] bg-[var(--error-bg)] px-4 py-3 text-sm text-[var(--error-text)]" role="alert">{importError}</p> : null}
      {incompleteDrafts ? <p className="mt-4 rounded-[5px] border border-[var(--amber-border)] bg-amber-soft px-4 py-3 text-sm text-[var(--amber-strong)]" role="status">有 {incompleteDrafts} 场多人对局已超过服务器保留期，无法恢复完整终态，未计入胜率与图表。</p> : null}

      <section className="grid grid-cols-4 gap-3 py-5 max-[900px]:grid-cols-2 max-[520px]:grid-cols-1" aria-label="总体指标">
        <Metric icon={BarChart3} label="总游玩" value={String(metrics.plays)} detail={`${metrics.losses} 负 · ${metrics.draws} 平`} />
        <Metric icon={Trophy} label="成功次数" value={String(metrics.wins)} detail={`胜率 ${percent(metrics.winRate)}`} tone="jade" />
        <Metric icon={Clock3} label="题局用时" value={formatDuration(metrics.medianMs)} detail={`平均 ${formatDuration(metrics.averageMs)} · P90 ${formatDuration(metrics.p90Ms)}`} />
        <Metric icon={CalendarCheck2} label="每日连胜" value={String(streak.current)} detail={`历史最长 ${streak.longest} 天`} tone="amber" />
      </section>

      <ChartSection title="作品猜测情况" description="灰柱为答案出现题局，主题色柱为获胜题局，折线为作品胜率。">
        {works.length ? <StatsChart option={workOption} ariaLabel="各东方作品答案出现次数、获胜次数与胜率" className="h-[390px]" /> : <EmptyChart />}
      </ChartSection>
      ...
      <div className="mt-4 grid grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] gap-4 max-[900px]:grid-cols-1">
        <ChartSection title="获胜猜测次数" description="只统计成功题局。">
          {guessDistribution.length ? <StatsChart option={distributionOption} ariaLabel="获胜题局猜测次数分布" /> : <EmptyChart />}
        </ChartSection>
        <ChartSection title="耗时分布" description="有效前台时间，不含页面后台停留与请求等待。">
          <div className="grid grid-cols-2 gap-2 max-[680px]:grid-cols-1">
            <MiniHistogram title="单次猜测" bins={guessHistogram} option={histogramOption(guessHistogram)} />
            <MiniHistogram title="整局游戏" bins={roundHistogram} option={histogramOption(roundHistogram)} />
          </div>
        </ChartSection>
      </div>
      ...
      {clearOpen ? (
        <ConfirmDialog
          ...
          onConfirm={async () => { await clearStatistics(); setClearOpen(false); }}
        />
      ) : null}
      {importPreview ? (
        <ImportDialog preview={importPreview} onClose={() => setImportPreview(null)} onApply={applyImport} />
      ) : null}
    </main>
  );
}

function ActionButton({ icon: Icon, label, danger = false, onClick }: { icon: typeof Download; label: string; danger?: boolean; onClick: () => void }) {
  return <button type="button" className={`inline-flex h-9 items-center gap-2 rounded-[5px] border px-3 text-xs font-bold ${danger ? "border-[var(--error-border)] text-[var(--error-text)] hover:bg-[var(--error-bg)]" : "border-line text-ink hover:border-line-strong hover:bg-[var(--surface-soft)]"}`} onClick={onClick}><Icon size={16} aria-hidden="true" />{label}</button>;
}
...
function Metric({ icon: Icon, label, value, detail, tone = "accent" }: { icon: typeof Target; label: string; value: string; detail: string; tone?: "accent" | "jade" | "amber" }) {
  const toneClass = tone === "jade" ? "text-jade bg-jade-soft" : tone === "amber" ? "text-amber bg-amber-soft" : "text-vermilion bg-vermilion-soft";
  return <article className="flex min-h-[112px] items-start gap-3 rounded-[7px] border border-line bg-[var(--surface)] p-4 shadow-sm"><span className={`inline-flex size-9 shrink-0 items-center justify-center rounded-[5px] ${toneClass}`}><Icon size={19} aria-hidden="true" /></span><div className="min-w-0"><p className="text-xs font-bold text-ink-soft">{label}</p><strong className="mt-1 block text-2xl text-ink">{value}</strong><small className="mt-1 block text-xs text-ink-soft">{detail}</small></div></article>;
}

function ChartSection({ title, description, children }: { title: string; description: string; children: React.ReactNode }) {
  return <section className="rounded-[7px] border border-line bg-[var(--surface)] p-4 shadow-sm"><div className="mb-2"><h2 className="text-base font-black text-ink">{title}</h2><p className="mt-0.5 text-xs text-ink-soft">{description}</p></div>{children}</section>;
}

function EmptyChart() { return <div className="flex h-[280px] items-center justify-center text-sm text-ink-soft">暂无符合筛选条件的数据</div>; }

function MiniHistogram({ title, bins, option }: { title: string; bins: ReturnType<typeof buildHistogram>; option: EChartsCoreOption }) {
  return <div className="min-w-0"><h3 className="px-2 pt-2 text-center text-xs font-bold text-ink-soft">{title}</h3>{bins.length ? <StatsChart option={option} ariaLabel={`${title}耗时直方图`} className="h-[280px]" /> : <div className="flex h-[280px] items-center justify-center text-xs text-ink-soft">暂无数据</div>}</div>;
}
...
function Outcome({ outcome }: { outcome: StatsOutcome }) {
  const style = outcome === "win" ? "bg-jade-soft text-jade" : "border border-[var(--error-border)] bg-[var(--error-bg)] text-[var(--error-text)]";
...
function ConfirmDialog({ title, text, confirmLabel, onCancel, onConfirm }: { title: string; text: string; confirmLabel: string; onCancel: () => void; onConfirm: () => void | Promise<void> }) {
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 px-4" role="presentation"><section className="w-full max-w-[430px] rounded-[7px] border border-line bg-[var(--surface)] p-5 shadow-lg" role="dialog" aria-modal="true" aria-labelledby="confirm-title"><div className="flex items-start justify-between gap-3"><div><h2 id="confirm-title" className="text-lg font-black text-ink">{title}</h2><p className="mt-2 text-sm leading-6 text-ink-soft">{text}</p></div><button type="button" className="inline-flex size-8 shrink-0 items-center justify-center rounded-[5px] text-ink-soft hover:bg-[var(--surface-soft)]" title="关闭" aria-label="关闭" onClick={onCancel}><X size={18} /></button></div><div className="mt-5 flex justify-end gap-2"><button type="button" className="h-9 rounded-[5px] border border-line px-4 text-sm font-bold text-ink" onClick={onCancel}>取消</button><button type="button" className="h-9 rounded-[5px] bg-vermilion px-4 text-sm font-bold text-[var(--accent-contrast)]" onClick={() => void onConfirm()}>{confirmLabel}</button></div></section></div>;
}

function ImportDialog({ preview, onClose, onApply }: { preview: { total: number; additions: number; replacements: number }; onClose: () => void; onApply: (mode: "merge" | "replace") => void | Promise<void> }) {
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 px-4"><section className="w-full max-w-[460px] rounded-[7px] border border-line bg-[var(--surface)] p-5 shadow-lg" role="dialog" aria-modal="true" aria-labelledby="import-title"><div className="flex items-start justify-between"><div><h2 id="import-title" className="text-lg font-black text-ink">导入统计数据</h2><p className="mt-1 text-sm text-ink-soft">已校验 {preview.total} 条记录</p></div><button type="button" className="inline-flex size-8 items-center justify-center rounded-[5px] text-ink-soft" aria-label="关闭" title="关闭" onClick={onClose}><X size={18} /></button></div><dl className="mt-4 grid grid-cols-2 gap-3"><div className="rounded-[5px] bg-jade-soft p-3"><dt className="text-xs font-bold text-jade">新增</dt><dd className="mt-1 text-xl font-black text-ink">{preview.additions}</dd></div><div className="rounded-[5px] bg-amber-soft p-3"><dt className="text-xs font-bold text-amber">同 ID 更新</dt><dd className="mt-1 text-xl font-black text-ink">{preview.replacements}</dd></div></dl><p className="mt-4 text-xs leading-5 text-ink-soft">合并会保留其他现有记录；覆盖会清空现有统计后导入，并再次要求确认。</p><div className="mt-5 flex flex-wrap justify-end gap-2"><button type="button" className="h-9 rounded-[5px] border border-line px-4 text-sm font-bold text-ink" onClick={onClose}>取消</button><button type="button" className="h-9 rounded-[5px] border border-vermilion px-4 text-sm font-bold text-vermilion" onClick={() => void onApply("replace")}>覆盖导入</button><button type="button" className="h-9 rounded-[5px] bg-jade px-4 text-sm font-bold text-white" onClick={() => void onApply("merge")}>合并导入</button></div></section></div>;
}
