...
- 建立 `RelayStageView`/selector，把当前 stage、encounters、viewer capability、历史选择和 standings 从 `RoomPage` 分离。
...
- 本地统计升级到 v6，匿名记录 relay scoring mode、最终名次/存留轮数、每 stage 积分变化、bye 和本人 encounter outcome；不保存房间/成员/对手身份。
...
- stats schema/migration/import/export、component tests、hook tests、Playwright 与视觉基线。
...
- stats v1-v6 导入后统一为 v6；导出不包含 `roomId/memberId/encounterId`、昵称、seat 或 token。
- desktop 与 Pixel 7 视觉/e2e 无页面横向溢出、按钮文字截断、积分条遮挡或棋盘/输入重叠。

## 可能涉及的代码

`apps/web/src/components/{RoomPage.tsx,RelayMatchBoard.tsx,MemberScoreStrip.tsx,MatchResultOverlay.tsx,GuessInputBar.tsx}`、可新增 `RelayStageView/EncounterPaginator/StageHistory`、`apps/web/src/hooks/useRoom.ts`、`apps/web/src/domain/`、`apps/web/src/stats/`、`apps/web/e2e/multiplayer.spec.ts`。
