...
**决策依据**：[房间配置与开局](./decisions.md#3-房间配置与开局)
...
- relay 创建请求接受 `playerLimit=2/4/6/8` 和 `eliminationEnabled`；缺省分别为 2 和 false。
...
- 开局事务按实际人数冻结 `legacy_wins`、`relay_points` 或 `relay_elimination`，房间开关之后不能改写当前 match。
...
- room policy、创建/settings/ready/start handler、数据库配置字段和权威 room projection。
- OpenAPI/WS room settings 字段、错误码、生成物和 API/并发测试。
- playerLimit 与 join/claim-seat/settings/final-ready 的竞争测试。
- 实际人数为 2 时忽略淘汰偏好并冻结 legacy scoring mode 的测试。
...
- relay 2 人在开关 true/false 下均产生 `legacy_wins`，现有 BO 目标和 turn 行为无回归。
