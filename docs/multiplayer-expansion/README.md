...
当前数据库约束保证玩家席位在房间内唯一，`player` 必须占用 `slot 1/2`，`spectator` 不占玩家席位。当前公开视图没有 `memberId`，人数、胜者、比分、棋盘和浏览器本地多人统计也都写死为 1/2；这些是 MPX-002 至 MPX-006 的改造范围。
...
所有改变房间状态的命令先在 Postgres 事务中完成，再写入 `room_event`，最后由 hub 广播。WebSocket 断线重连时携带 `lastSequence`；协议要求 sequence 缺口通过 snapshot 补齐，但当前 `useRoom` 只去重并推进水位，没有严格检查 `sequence === lastSequence + 1`，且服务端会对猜测者跳过自己的猜测事件。MPX-002 必须一起修正这两个问题，不能直接把现状当作可靠的连续流。
...
当前的“玩家看自己/匿名对手、观战者看完整棋盘”由服务端投影保证，前端不能自行恢复被隐藏字段。聊天仍未实现；本轮由服务端根据发送者身份固定选择 `room` 或 `spectator` channel，客户端不能提交任意 scope，详见 MPX-007 至 MPX-009。
...
| `minPlayers` | 允许开局的最少玩家数 | 本轮由服务端固定为 2，不开放房主设置；当前玩家达到下限且全员连接、准备后，以当时阵容开局 |
...
`playerLimit` 字段本身不要求升级 WebSocket 版本；真正需要 v2 的是本轮同时引入了以下不兼容变化：
...
因此 v2 的目的不是给 `playerLimit` 换名字，而是让服务端在握手阶段明确拒绝不理解新集合和重放语义的旧客户端，避免“连接成功但状态错误”的隐性不兼容。本轮 wire 集合统一为包含 `memberId` 的数组并按 seat 稳定排序，客户端再按 memberId 建索引；不保留 `slot1/slot2` 双写，也不使用动态 JSON key。

理论上可以在 v1 中增加可选字段并长期维护两套 payload，但当前多人分支尚未合入 `main`，房间数据也只短期保留，双协议适配、测试和排障成本高于兼容收益。因此本计划选择直接使用 `touhouflandre-multi.v2`，部署时停止创建 v1 房间并等待其排空或关闭，同时向旧页面显示刷新提示；具体演练由 MPX-010 验收。
...
| M1 共享基础 | MPX-002、MPX-003 | memberId/seat、WS v2、容量和房间生命周期 | 001；003 依赖 002 |
| M2A 服务端竞速链（协作者 A） | MPX-004、MPX-005 | N 人竞速规则、计分、玩家上限 API | 003；005 依赖 004 |
| M2B 服务端聊天链（协作者 A） | MPX-007、MPX-008 | 独立聊天游标、消息持久化与授权投影 | 002；008 依赖 007 |
...
按下面顺序从当前集成分支推进。每个步骤都从最新的 `feature/multipalyer_mode_backend` 创建短期分支，通过本阶段闸门后合并回集成分支，再开始依赖它的下一步。

| 步骤 | Issue / 执行内容 | 负责人 | 进入条件 | 完成闸门 |
|---|---|---|---|---|
| 0. 基线确认 | 确认当前双人 race/relay、观战、WS 重放和本地统计测试基线 | 两人共同 | 当前集成分支可构建 | 基线测试结果已记录，工作区生成物无未解释漂移 |
| 1. 冻结决策 | MPX-001：冻结 `playerLimit`、memberId/seat、roster、权限、聊天 channel 和 v2 原因 | 两人共同评审，A 落文档 | 步骤 0 完成 | 状态/权限/可见性矩阵无待定安全语义，002—010 均引用同一口径 |
| 2. 建立协议与数据底座 | MPX-002：memberId/seat、`player_limit`、WS v2 集合、cursor 与同步屏障 | A 实现，B 审查客户端状态 | MPX-001 合并 | 两人 race/relay 在 v2 下完整回归；迁移、生成物和 sequence 缺口测试通过 |
| 3. 接入共享生命周期 | MPX-003：join/claim-seat、ready/unready、容量、开局冻结、rematch 和 spectator cap | A 实现，B 审查契约 | MPX-002 合并 | 并发入座/最终 ready 可串行化；创建 `mpx-003-base` 共享基线 |
...
| 5B. 交付聊天服务端 | MPX-007 → MPX-008：先用文档 PR 冻结聊天契约，再由 MPX-008 在一个实现 PR 中同步落地生效契约、生成物、持久化、授权、历史和实时同步 | A | 操作上等待 MPX-005 合并；逻辑基线为 MPX-002 | channel 权限、幂等、cursor、重连无缺口和 XSS/限流测试通过 |
| 6. 交付聊天 Web | MPX-009：聊天面板、历史、重连、未读和 `receiveChat` | B | MPX-008 合并；可与已完成的 MPX-006 汇合 | 玩家/观战者可见性、角色变化、闭麦、断线和移动端 e2e 通过 |
| 7. 集成与发布闸门 | MPX-010：迁移、并发、安全、性能、灰度、回滚、文档和用户更新公告 | 两人共同 | MPX-006、009 均合并 | 全量检查、v1 房间排空和生产回滚演练完成；用户公告评审通过，并在默认开放时同步发布，之后才完成发布流程 |

步骤 5A 与 5B 是本路线图的主要并行窗口：MPX-005 合并后，B 开始 MPX-006，A 同时按 MPX-007 → MPX-008 推进；B 在 MPX-008 合并后再开始 MPX-009。功能依赖链分别为 `001 → 002 → 003 → 004 → 005 → 006 → 010` 和 `001 → 002 → 007 → 008 → 009 → 010`；为避免 A 的契约、SQL 和生成代码冲突，实际合并顺序固定为 `001 → 002 → 003 → 004 → 005 → (006 ∥ 007 → 008) → 009 → 010`。

执行中发现问题时回到拥有该决策的 Issue 修正：房间/N 人规则回到 MPX-003—005，聊天授权或游标回到 MPX-007/008，Web 不用本地兼容分支绕过服务端契约；MPX-010 只承担集成修复和发布验收，不在最后阶段重新设计核心规则。
...
flowchart TD
    ...
    A5 --> A7["协作者 A：feat/mpx-007-chat-policy<br/>操作性串行，逻辑上只依赖 MPX-002"]
...
2. MPX-001 由两人共同评审；MPX-002、MPX-003 由协作者 A 主实现，协作者 B 负责契约、迁移和 v2 客户端状态审查。共享基础未通过前，不开始功能实现。
3. 协作者 A 负责所有服务端与协议工作，并按 `MPX-004 → MPX-005 → MPX-007 → MPX-008` 的顺序合并。MPX-007 逻辑上只依赖 MPX-002，但操作上排在 MPX-005 后，避免多个分支同时修改 OpenAPI/WS、SQL 和生成代码。
...
8. 每次合并后在集成分支运行至少 `pnpm typecheck`、`pnpm test`、`pnpm lint:openapi`、`pnpm check:openapi-refs`、`pnpm check:ws-protocol` 和 `cd apps/api && go test ./...`。涉及契约或 SQL 时运行 `task gen`，并确认 `apps/api/internal/generated` 与 `apps/web/src/generated` 均无未预期漂移；当前 `task check:generated` 只覆盖 Go 生成目录，不能替代 Web 生成物检查。
...
| [MPX-002](./MPX-002-participant-seat-team-foundation.md) | 建立 member/seat 与 WS v2 数据底座 | `type:feature`, `area:db`, `area:contracts` | 集合不再写死 1/2，投影序列可连续校验 |
| [MPX-003](./MPX-003-room-lifecycle-and-team-assignment.md) | 将容量、入座与灵活开局接入房间生命周期 | `type:feature`, `area:api`, `area:multi` | 共享房间状态机闸门 |
...
- 管理员踢人、禁言、举报和聊天审计后台；本轮协议也不预留可由客户端选择的 `team`/`member` 私聊 scope。
