...
创建页只在 race 显示 2..8 连续人数滑杆，大厅把 relay 写成“固定 2 人”，也没有淘汰设置或奇数人数阻塞反馈。需要在不复制 race 表单逻辑的前提下，为 relay 暴露模式策略驱动的控件。

## 要做到什么程度

- 抽取可复用的 `PlayerLimitControl`，由 mode policy 提供 allowed values、min/max/step 和文案；race 保持 step 1，relay 使用 step 2。
...
- `MultiLobby` 创建表单、`RoomLobby` 房主设置和共享人数/开关组件。
...
- 创建与大厅 switch 默认/保存/刷新/重连状态一致；实际 2 人开局时 UI 结果仍显示传统 BO。
...
`apps/web/src/components/{MultiLobby.tsx,RoomLobby.tsx,PlayerLimitControl.tsx}`、`apps/web/src/config/multiplayerRollout.ts`、`apps/web/src/lib/api.ts`、`apps/web/src/domain/multiRoom.ts`、相关 component/e2e tests。
