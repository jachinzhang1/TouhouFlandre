...
- [x] 根布局内始终只有一个音频元素；打开/关闭任何播放器 UI 不改变其引用。
- [x] 首页播放后通过 `Link` 依次进入至少两个其他页面，`currentSrc` 不变、时间继续前进且没有额外暂停。
- [x] 刷新后重新加载默认或已注入的初始曲目/音量，但保持暂停且从 0 秒开始；localStorage 读取留给 MUS-006。
- [x] 上一首在首项回到末项，下一首在末项回到首项，`ended` 与下一首使用同一循环逻辑。
- [x] 暂停时切歌仍暂停；播放时切歌在允许的情况下继续播放。
- [x] 快速连续切歌不会被旧曲目的 metadata/error 事件回滚。
- [x] `duration=NaN/Infinity/0`、越界 seek 和加载失败均不会产生异常或无效 CSS 数值。
- [x] 播放失败后状态与实际 audio 一致，用户仍可暂停、重试、切歌或打开设置。
- [x] 音量图标所需等级由纯函数派生；静音前后的音量恢复符合决策文档。
- [x] 单元测试不依赖真实网络；真实媒体和导航连续性由 Playwright 覆盖。
...
- **完成日期**：2026-08-20
- **播放内核**：根级 `MusicPlayerProvider` 独占唯一 audio adapter，通过 reducer 归一化媒体事件、队列、播放意图、时间、音量和错误状态。
- **目录与接口**：Web 目录适配器按专辑顺序、曲号和稳定 ID 派生队列；`useMusicPlayer` 只暴露 MUS-001 冻结的视图状态与命令。
- **生命周期**：source generation 拦截旧事件，Strict Mode cleanup 无残留 listener；软导航保持同一 audio/currentSrc/currentTime，硬刷新不自动续播。
- **自动化证据**：music-player 27 个 Vitest、Web 全量 168 个 Vitest、production build，以及 Desktop Chromium/Pixel 7 的真实 MP3、Range、自然结束、软导航和硬刷新用例通过。
- **依赖结果**：未新增 npm 依赖，未修改锁文件；版本化 localStorage 读取与写入继续由 MUS-006 实现。
