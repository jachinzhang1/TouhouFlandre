...
- 新增房间级消息表（推荐 `multi_chat_message`）和必要索引，文本/emoji 按 MPX-007 冻结的 `kind + content` 保存；使用稳定、可分页的独立 chat cursor，消息写入数据库后再广播，但不写 `room_event`。消息随 room 删除而清理，但 senderMemberId/displayName/role/seat 作为不可变快照，不对活动成员行设置级联删除依赖，不能因 lobby 玩家行被删除而丢失消息。
- 实现固定的 `POST /api/rooms/{roomId}/messages` 与 `GET /api/rooms/{roomId}/messages`；GET 以无 cursor、`after` 或 `before` 三种互斥形态提供初始历史、重连补齐和更早分页。发送使用成员 token、clientMessageId 和服务端 sender 推导，数据库以 `(room_id, sender_member_id, client_message_id)` 保证幂等唯一。
...
- lobby/playing/finished 中的 connected member 允许发送；retained left member 在 finished 保留期内只能按最后有效 role 读取获授权历史，不得发送。closed、过期或已清理房间均不再提供聊天读写，即使物理消息行尚未清理。
...
- 重连和分页历史严格按授权返回；player token 永远不能读取 `spectator` channel；retained left token 仅在 finished 保留期内按最后有效 role 读取历史，发送稳定拒绝，closed 后读写均拒绝。
...
首个聊天迁移使用 `0012`；`0011_race_elimination_scoring.sql` 已由 MPX-006 占用。实现必须以 MPX-007 在 `decisions.md` 冻结的请求/响应、错误码和双水位屏障为准，不得在 handler 或生成代码中临时发明另一套 wire 语义。
