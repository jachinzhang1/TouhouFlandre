...
单人和多人对局会按输入匹配程度排列现有命中项：完全匹配优先于前缀匹配，前缀匹配优先于其他连续子串；同级再比较匹配位置和词条长度差，最后按初登场顺序与角色 ID 稳定排序。该算法不容忍错别字或漏字，也不区分正式姓名、别名和作品信息的字段权重。排序逻辑在 Go 与 TypeScript 中以独立纯模块实现，并由同一份黄金样例约束，便于替换或扩展而不改变搜索范围。
