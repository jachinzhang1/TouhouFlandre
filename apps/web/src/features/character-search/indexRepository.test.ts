...
describe("CatalogSearchIndexRepository", () => {
  ...
  it("preserves structured missing-version and compatibility route codes", async () => {
    const missingVersion = new CatalogSearchIndexRepository({
      fetch: vi.fn().mockResolvedValue(
        new Response(JSON.stringify({ code: "CATALOG_VERSION_NOT_FOUND" }), {
          status: 404,
        }),
      ),
    });
    await expect(missingVersion.load("missing")).rejects.toMatchObject({
      status: 404,
      code: "CATALOG_VERSION_NOT_FOUND",
    });

    const oldRoute = new CatalogSearchIndexRepository({
      fetch: vi
        .fn()
        .mockResolvedValue(new Response("not found", { status: 404 })),
    });
    await expect(oldRoute.load("catalog-v1")).rejects.toMatchObject({
      status: 404,
