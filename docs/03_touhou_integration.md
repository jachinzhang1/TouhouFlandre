...
本文规定东方芙一把（TouhouFlandre）题库的字段、标准化方式、来源要求和贡献流程。目标是让反馈结果可复现、可解释，并减少不同译名或设定解读造成的歧义。
...
客串、背景出现、后续设定补充等边界情况应在贡献说明中给出来源与判定理由。
...
7. 运行 `pnpm test` 与 `cd apps/api && go test ./...`，确认数据、搜索与比较逻辑测试通过；
8. 在提交说明中列出涉及字段的资料来源；
...
头像文件名开头的四位数字用于派生 `appearanceOrder`，不在角色 JSON 中重复维护。格式为“二位作品序号 + 一位整数/小数点作标记 + 一位作品内登场顺序”：第三位为 `0` 表示整数作、为 `5` 表示小数点作；`0601` 表示第 6 作第 1 个登场的新角色，`0751` 表示第 7.5 作第 1 个登场的新角色，`00xx` 用于旧作或非游戏作品。编号必须唯一，路径必须符合 `/characters/NNNN-名称.png` 形式，其中 `NNNN` 为四位数字。

头像素材包可以包含尚未进入题库的角色。校验器要求每条已收录记录的头像存在，但不会把未引用头像视为错误；这些文件作为后续题库扩充的预留资源保留。

首页封面图的原始来源和许可尚未确认，状态记录在 [`THIRD_PARTY_ASSETS.md`](../THIRD_PARTY_ASSETS.md)。该图片不属于 MIT 授权范围，也不应在项目外重新分发。除该项待确认资源外，不得引入原作提取图片、游戏截图或来源不明的同人图片。引入图像资源时，必须同时记录作者、原始地址、许可证与必要的署名方式，并确认许可证允许仓库分发和网站使用。
