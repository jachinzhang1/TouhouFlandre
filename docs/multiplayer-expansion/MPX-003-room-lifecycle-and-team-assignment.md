...
**依赖**：MPX-002C（前置子任务 MPX-002A、MPX-002B 已完成）
...
- lobby 的 ready/start 条件和 host close 使用活动玩家集合，而不是固定两个成员；seat 1 继续作为房主判断，memberId 作为身份键。finished rematch 只允许本场冻结 roster 完整、无人 left 且 roster 全员 connected + rematch ready 时按原 roster 开新场，新成员或 spectator 不能补位。
- `room.updated`、room info、snapshot 返回 `playerLimit`、`minPlayers`、玩家数、可用席位、spectator 数和带 memberId 的成员集合，让前端不再猜测身份。
