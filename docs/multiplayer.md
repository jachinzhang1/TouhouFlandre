...
多人房间支持 BO1、BO3、BO5 和 BO7 双人赛制。竞速房间允许 2..8 个玩家席位，接力房间固定两人。房主创建房间并选择玩法模式和竞速人数上限，加入者通过 6 位房间号进入；至少有两名玩家且当前玩家全员准备后进入倒计时并冻结本场 roster。`playerLimit` 只是容量上限，实际开局人数 N 决定竞速规则：N=2 使用所选 BO 赛制，N>2 使用积分淘汰制。开局后新加入者始终是观战者，淘汰或离场产生的空缺不会重新开放。原 roster 完整、在线且全员确认后可再来一局。

| 模式       | 标识    | 规则                                                                                           |
| ---------- | ------- | ---------------------------------------------------------------------------------------------- |
| 双人竞速   | `race`  | 两名玩家同时竞猜，率先猜中者赢得本局，先达到 BO 目标胜场者赢得整场。                           |
| 多人竞速   | `race`  | 三至八名玩家同时竞猜；每局按完成顺序计分并从指定轮次开始淘汰，最终按累计积分生成完整排行榜。   |
...
大厅中的加入者离开会释放座位；对局中的离开会保留成员行用于结算和终态恢复。积分场的 match player 另有 `active`、`eliminated`、`left` 状态；淘汰者仍保留 player 身份和聊天权限，但游戏操作与棋盘投影切换为只读观战模式。
...
1. 每局开始时冻结当前 `active` 人数 n。第 m 名猜中者获得 `n-m+1` 分；猜中后本人停止行动，但本局继续。
2. 放弃、离场、次数耗尽或 5 分钟截止时仍未猜中者得 0 分。所有参与者进入 `correct`、`forfeited`、`exhausted` 或 `timed_out` 后统一结算。
3. 从第 `floor(N/2)` 局结算后开始淘汰：先取累计积分最低者，再取其中历史最高单局积分最低者；仍并列则同时淘汰。若候选包含全部 active 玩家，本局不淘汰。
4. 离场或断线宽限逾期会永久标记 `left` 且本局得 0 分；若仍有多人存活，本局继续并在局末执行正常积分淘汰。
5. 剩余不超过一人、剩余两人且积分差大于 1，或达到 `3N` 局安全上限时结束整场。
6. 完整排行榜按总积分降序并使用 `1、1、3` 式共享名次。唯一第一名是冠军；并列第一时所有第一名为 draw，其余为 loss，不设置单一 `winnerMemberId`。

服务重启或优雅排空会明确结束当前局和整场；积分场仍发布当前参与状态、0 分记录和完整排行榜。
...
| 模式 | 进行中可见内容                                                   | 局末可见内容                                                |
| ---- | ---------------------------------------------------------------- | ----------------------------------------------------------- |
| 竞速 | active 玩家看到自己的完整猜测；其他玩家只显示匿名矩阵。淘汰者改为只读完整棋盘。 | 揭示答案、全员比分、每局积分、淘汰状态、所有完整棋盘和结算结果。 |
...
| `match.started`        | 新对局开始，携带赛制、模式、`scoringMode`、`rosterSize`、`maxRounds` 和题库版本。                                  |
...
| `round.ended`          | 小局结束，揭示答案、比分、棋盘；积分场另含每人状态、名次、得分和本局淘汰集合。                                    |
| `match.ended`          | 整场对局结束；积分场另含完整共享名次排行榜。                                                                      |
...
- 双人竞速的并发正确猜测恰有一个局胜者；多人竞速并发正确猜测获得唯一完成名次且只计分一次。
- 多人竞速的排名积分、成组淘汰、全员同分不淘汰、离场、两人差距终止、`3N` 上限和共享冠军。
- 竞速模式重复猜测、猜测上限、局末后提交和超时。
- 接力模式共享棋盘、轮到玩家校验、猜测推进、主动空过、超时空过和空过额度。
- 当前小局主动放弃、断线宽限、重连、事件重放和 snapshot 补齐。
- 竞速匿名矩阵不泄露角色名或标签值。
- N 人棋盘分页只挂载当前页；进行中的 active 玩家始终只挂载一张对手棋盘，局末和观战桌面每页两张、移动端每页一张。
