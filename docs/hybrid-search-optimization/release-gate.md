...
| 2 全部  | Go `search_fixtures_test.go` 与 Web `engine.test.ts` 共同消费 `search-parity-v2.json`；`search-ranking-v2.json` 额外约束字段优先级及匹配类型优先语义；Playwright 从真实 catalog、random session、race match 和 relay match 装配版本/范围，以两组查询比较本地内核与 Go 远程的 ID 顺序和 total；空 scope fail closed。                                                                                                                                                                                                                                                         |
