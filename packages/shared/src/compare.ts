import { GUESS_FIELDS, HAIR_COLOR_LABELS } from "./fields";
import type { Character, FeedbackStatus, FieldFeedback, GuessField, GuessFieldKey, GuessResult } from "./types";
...
const compareMultiSet = (guessValues: string[], answerValues: string[]): FeedbackStatus => {
...
const displayValuesForField = (character: Character, field: GuessFieldKey): string[] => {
  if (field === "firstAppearance") return [character.firstAppearance.workTitle];
  if (field === "releaseYear") return [String(character.firstAppearance.releaseYear)];
  if (field === "hairColors") return character.hairColors.map((color) => HAIR_COLOR_LABELS[color]);
  return character[field];
};

const valuesForField = (character: Character, field: GuessFieldKey): string[] => {
...
export const compareField = (guess: Character, answer: Character, field: GuessField): FieldFeedback => {
  ...
  if (field.compareStrategy === "multiSet") {
    status = compareMultiSet(valuesForField(guess, field.key), valuesForField(answer, field.key));
  }
...
export const compareCharacter = (
  guess: Character,
  answer: Character,
  fields: GuessField[] = GUESS_FIELDS
): GuessResult => ({
  guessId: guess.id,
  guessName: guess.names.zhHans,
  isCorrect: guess.id === answer.id,
  feedback: fields.filter((field) => field.visible).map((field) => compareField(guess, answer, field))
});
