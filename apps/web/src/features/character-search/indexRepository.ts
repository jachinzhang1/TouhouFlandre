...
export class CatalogSearchIndexRepository {
  ...
  load(catalogVersion: string, indexSchemaVersion = 1, signal?: AbortSignal, policyRevision = "v1"): Promise<CatalogSearchIndex> {
    const key = `${catalogVersion}:${indexSchemaVersion}` as Key;
    const existing = this.values.get(key);
    if (existing) return this.consumerPromise(Promise.resolve(existing), key, signal);
    let shared = this.inFlight.get(key);
    if (!shared) {
      const controller = new AbortController();
      this.controllers.set(key, controller);
      shared = this.fetchAndValidate(catalogVersion, indexSchemaVersion, undefined, controller.signal).catch(async (error) => {
        if (!(error instanceof Error) || !["INVALID_INDEX", "UNSUPPORTED_SCHEMA", "VERSION_MISMATCH"].includes((error as { code?: string }).code ?? "")) throw error;
        return this.repair(key, catalogVersion, indexSchemaVersion, policyRevision);
      });
      this.inFlight.set(key, shared);
      void shared.then((value) => this.values.set(key, value), () => undefined).finally(() => {
  ...
  private async repair(key: Key, catalogVersion: string, schemaVersion: number, policyRevision: string): Promise<CatalogSearchIndex> {
    const repairKey = `${key}:repair:${policyRevision}`;
    if (this.repairAttempts.has(repairKey)) {
      throw new SearchIndexValidationError("INVALID_INDEX", "index repair already attempted");
    }
    this.repairAttempts.add(repairKey);
    if (this.inFlight.has(repairKey as Key)) return this.inFlight.get(repairKey as Key)!;
    const promise = this.fetchAndValidate(catalogVersion, schemaVersion, "reload").finally(() => this.inFlight.delete(repairKey as Key));
  ...
  private async fetchAndValidate(version: string, schemaVersion: number, cache?: RequestCache, signal?: AbortSignal): Promise<CatalogSearchIndex> {
    ...
    try {
      ...
      throw new SearchIndexValidationError("INVALID_INDEX", "search index JSON is invalid");
    }
    return validateSearchIndex(body, version, schemaVersion);
  }

  private consumerPromise(promise: Promise<CatalogSearchIndex>, key: Key, signal?: AbortSignal): Promise<CatalogSearchIndex> {
    ...
    return new Promise((resolve, reject) => {
      ...
      promise.then((value) => { if (!settled) { release(); resolve(value); } }, (error) => { if (!settled) { release(); reject(error); } });
    });
  }
}
