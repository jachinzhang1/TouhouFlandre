...
const guessSchema = characterSchema.extend({ durationMs: z.number().nonnegative().optional(), correct: z.boolean() });
...
const exportSchema = z.object({ schemaVersion: z.literal(STATS_SCHEMA_VERSION), exportedAt: z.string(), records: z.array(z.discriminatedUnion("kind", [singleSchema, multiplayerSchema])) });

export async function createStatsExport(): Promise<StatsExportFile> {
  return { schemaVersion: STATS_SCHEMA_VERSION, exportedAt: new Date().toISOString(), records: await statsDb.records.toArray() };
}
...
export function parseStatsImport(text: string): StatsExportFile {
  ...
  return parsed as StatsExportFile;
}
