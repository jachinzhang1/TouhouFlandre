...
- [x] 卡片包含需求列出的全部信息与控制，且没有创建第二套播放状态或 audio 元素。
- [x] 3 首测试曲目都显示正确标题、专辑/艺人和封面；模拟封面失败时一次回退占位图。
- [x] 短标题静止，长标题仅在溢出时循环；resize/换歌后判定更新，屏幕阅读器不重复朗读。
- [x] 进度拖动不会被 `timeupdate` 抢回；键盘方向键也可 seek。
- [x] 上/下一首、播放/暂停和媒体错误状态与 Provider 一致。
- [x] 音量图标在 0、低、中、高和 muted 时正确，点击图标能恢复静音前音量。
- [x] 切换六种主题色与深/浅模式时，卡片、歌曲进度和音量填充即时更新。
- [x] 展开/收起、外部点击和 Escape 行为确定，关闭后焦点回到入口。
- [x] 320px 和 Pixel 7 下无横向溢出、文本遮挡或小于合理触控尺寸的主要按钮。
- [x] reduced motion 下无位移/缩放和自动跑马灯，但所有功能仍可使用。
...
- **完成日期**：2026-08-21
- **组件实现**：新增 `PlayerCard`、`TrackCover`、`MarqueeTitle`，接入 `MusicPlayerRoot` 的展开/收起、外部点击、Escape 和焦点返回；封面失败可回退到本地占位图，曲目切换会重置回退状态。
- **播放控制**：复用 MUS-003 的 Provider 命令实现播放/暂停、首尾切歌、进度提交、音量和静音；未新增 `<audio>` 或第二套播放状态。
- **主题与响应式**：复用站点 CSS 变量，覆盖 320px、桌面、Pixel 7、深浅色和六种主题色；使用 `ariaLabelForHandle` 暴露 Ant Design Slider 的中文无障碍名称。
- **自动化证据**：Web typecheck 通过；Web Vitest `39` 个文件、`182` 个测试通过；production build 通过；MUS-005 Playwright Desktop Chromium/Pixel 7 `10/10` 通过；播放器相关 MUS-001~005 回归 `22/22` 通过。
- **依赖与资产**：未新增 npm 依赖，未修改音乐数据、MP3、封面或授权清单。
