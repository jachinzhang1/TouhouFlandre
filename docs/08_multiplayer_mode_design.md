...
- 排行榜、对局回放、云端对局保存、服务端个人统计（浏览器可按终态事件建立不上传的本地个人统计，见 §5.3）。
...
5. **游客身份房间级**：令牌只对该房间有效；服务器跨房间不累计统计或存档（无排行榜、无云端对局记录）。浏览器可在不上传身份凭据的前提下累计本地个人统计。
...
- v1 无排行榜（本就属规划中）；游客身份不产生任何服务端跨房间统计表，无对局历史查询接口，无回放。这是产品边界而非缺口：`multi_*` 表只承载房间生命周期与**场次记录**（房间关闭后经事件保留期由 sweeper 单条 `DELETE FROM multi_room` CASCADE 清理，见 §9.1），服务器不跨房间累计统计。
- Web 客户端可从 `round.ended` / `match.ended` 和终态 snapshot 建立浏览器 IndexedDB 本地统计。完成记录不保存 guest token、房间号或对手昵称，不上传服务器；`left` 成员令牌在保留期内只允许读取终态 snapshot，所有写命令与 WebSocket 仍拒绝。
...
| 物化视图/窗口函数/生成列 | 不采用 | 无服务端统计需求；`is_correct` 依赖跨表比较，生成列不适用 |
