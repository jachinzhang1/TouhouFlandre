...
| M5 曲目列表   | [MUS-008](./MUS-008-track-list-and-settings-entry.md)         | 已完成：齿轮设置入口、内嵌曲目列表、任意曲目播放、动效与无障碍 | MUS-003、005、006、007 |

MUS-004、MUS-005、MUS-006 可以从同一个 MUS-003 基线并行，但三者不得各自创建音频实例或各自维护当前曲目。MUS-007 只做集成修复和质量收口，不在最后阶段重写曲库或状态模型。MUS-008 继续消费现有 provider 契约，不新增页面层播放器状态或音频实例。
